...
from src.reward_model.kernels.linear import LinearKernel
from src.utils import multivariate_normal_sample
...
class LinearObservationGP:
    ...
    def _get_kernel_cholesky(self, obs_noise: float = None) -> np.ndarray:
        """Performs Cholesky factorization for the training kernel matrix.

        Args:
            obs_noise (float, optional): Obesrvation noise variance. Defaults to None.

        Returns:
            np.ndarray: Cholesky factor.
        """
    ...
    def A_inv(self):
        if self._A_inv is None:
            ...
            if conditioning_number > 1e7:
                print(f"Warning: high conditioning_number: {conditioning_number}")
