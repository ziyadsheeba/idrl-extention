...
from src.utils import get_array_from_scalar, save_video
...
class NonlinearQuery(QueryBase):
    def __init__(self, gp_repr_list, linear_combination, reward, info):
        self.gp_repr_list = get_array_from_scalar(gp_repr_list)
        self.linear_combination = get_array_from_scalar(linear_combination)
        self.reward_ = reward
...
class ComparisonQueryNonlinear(NonlinearQuery):
    def __init__(
        self, gp_repr1, gp_repr2, reward1, reward2, info, response="sigmoid", **kwargs
    ):
        gp_repr_list = [gp_repr1, gp_repr2]
        linear_combination = [1, -1]
        super().__init__(gp_repr_list, linear_combination, 0, info)
...
class StateComparisonQueryNonlinear(ComparisonQueryNonlinear):
    def __init__(self, gp_repr1, gp_repr2, reward1, reward2, info, response="sigmoid"):
        super().__init__(gp_repr1, gp_repr2, reward1, reward2, info, response=response)
